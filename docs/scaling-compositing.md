...
The `CdkPixBuf` class contains methods to scale pixbufs, to scale
...
Since the full-featured functions [method@CdkPixbuf.Pixbuf.scale],
[method@CdkPixbuf.Pixbuf.composite], and [`method@CdkPixbuf.Pixbuf.composite_color`]
are rather complex to use and have many arguments, two simple
convenience functions are provided, [`method@CdkPixbuf.Pixbuf.scale_simple`]
and [`method@CdkPixbuf.Pixbuf.composite_color_simple`] which create a new
...
acceleration on systems where MMX is supported. If `CdkPixbuf` is built
...
compositing, by using the contents of a `CdkPixbuf` pixel buffer as the
